...
export class AppController {
  constructor(
    ...
    @Inject('NOTIFICAION_SERVICE') private readonly rmqClient: ClientRMQ,
  ) {}

  @MessagePattern(ORDER_PATTERN.ORDER_CREATED)
  async process(@Payload() orderDto: CreateOrderDto) {
    const processedProducts =
      await this.appService.handleProcessProducts(orderDto);
    //notification
    this.rmqClient.emit(PRODUCT_PATTERNS.PRODUCT_PROCESSED, processedProducts);
  }
  ...
  create(@Payload() productCreateDto: CreateProductDto) {
    ...
    //notification
    this.rmqClient.emit(PRODUCT_PATTERNS.PRODUCT_CREATED, productCreateDto);
  }
  ...
  update(@Payload() updateProductDto: UpdateProductDto) {
    ...
    //notification
    this.rmqClient.emit(PRODUCT_PATTERNS.PRODUCT_UPDATED, updateProductDto);
  }
}
